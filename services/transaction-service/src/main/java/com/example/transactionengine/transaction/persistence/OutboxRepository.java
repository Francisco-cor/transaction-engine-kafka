...
public class OutboxRepository {
  ...
  public void insert(OutboxEvent event) {
    jdbc.update(
        ...
        INSERT INTO transaction_schema.outbox_events (
            aggregate_id, event_type, schema_version, payload, headers, partition_key
        ) VALUES (
            :aggregateId, :eventType, :schemaVersion, CAST(:payload AS JSONB),
            CAST(:headers AS JSONB), :partitionKey
        )
        """,
  ...
  public List<ClaimedOutboxEvent> claim(int batchSize, String owner, int leaseSeconds) {
    return jdbc.query(
        """
        WITH candidates AS (
            SELECT outbox_id
